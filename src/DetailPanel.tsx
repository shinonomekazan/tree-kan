...
interface DetailPanelProps {
  ...
  onUpdateNodeLinks: (nodeId: string, slackLinks?: LinkItem[], githubLinks?: LinkItem[]) => void;
...
const DetailPanel: FC<DetailPanelProps> = ({
  ...
  const handleAddChild = useCallback((payload: NewNodePayload) => {
  ...
  const handleAddOrEditLink = useCallback((link: LinkItem) => {
    if (!selectedNode) return;
    const isSlack = linkModalState.type === "slack";
    const currentLinks = isSlack ? selectedNode.slackLinks : selectedNode.githubLinks;
  ...
  const handleOpenLinkModal = useCallback((type: "slack" | "github", editLink?: LinkItem) => {
    setLinkModalState({ isOpen: true, type, editLink });
  }, []);

  const handleOpenDeleteConfirm = useCallback((type: "slack" | "github", id: string) => {
    setDeleteConfirmState({ isOpen: true, type, id });
  }, []);

  if (!selectedNode) {
    return <div className="w-64 h-full bg-white shadow-xl flex flex-col z-10 border-l border-slate-200 shrink-0" />;
  }
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {selectedNode.type === "task" && selectedNode.status && (
            <div className="flex items-center gap-2">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold text-white ${selectedNode.status === "done" ? "bg-blue-500" :
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-end">
              <button
                onClick={() => onUpdateDescription(selectedNode.id, description)}
          ...
          <div className="space-y-4">
            <LinkGroup
              ...
              iconNode={<img src={slackIcon} alt="Slack" className="w-5 h-5 object-contain" />}
            ...
            <LinkGroup
              ...
              iconNode={<img src={githubIcon} alt="GitHub" className="w-5 h-5 object-contain" />}
              onAdd={handleOpenLinkModal}
              onEdit={handleOpenLinkModal}
              onDelete={handleOpenDeleteConfirm}
            />
